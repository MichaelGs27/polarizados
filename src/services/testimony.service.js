...
exports.create = async (newTestimony) => {
    const [result] = await db.execute(
        'INSERT INTO testimonies (comment, date) VALUES (?, ?)',
        [newTestimony.comment, newTestimony.date]
    );
...
exports.update = async (idTestimony, updatedTestimony) => {
    const [result] = await db.execute(
        'UPDATE testimonies SET comment = ?, date = ?  WHERE idTestimony = ?',
        [updatedTestimony.comment, updatedTestimony.date, idTestimony]
    );
